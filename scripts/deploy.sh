...
echo "=============================="
echo "  LLM Fine-tune Platform - 部署"
echo "  目标: $DEPLOY_DIR"
echo "  时间: $TIMESTAMP"
echo "=============================="
...
echo ""
echo "[1/6] 准备目录..."
mkdir -p "$DEPLOY_DIR"/{data,backend/data,backend/logs}
...
if [ ! -f "$NGINX_CONF" ] || [ "$DEPLOY_FRONTEND" = true ]; then
    sed -e "s|{{NGINX_PORT}}|${NGINX_PORT}|g" \
        ...
        "$PACKAGE_DIR/scripts/finetune.conf" > "$NGINX_CONF"
...
if systemctl is-active --quiet "llm-finetune-backend" 2>/dev/null; then
    systemctl stop llm-finetune-backend
    systemctl disable llm-finetune-backend 2>/dev/null
...
echo ""
echo "[6/6] 健康检查..."
if curl -sf --max-time 10 "http://127.0.0.1:${BACKEND_PORT}/api/system/health" > /dev/null 2>&1; then
